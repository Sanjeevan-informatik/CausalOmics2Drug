...
This application is stateless and has no authentication or persistent database. Keep it on localhost for research development. A shared patient-data service would need an explicitly designed access and storage model. The hosted Sites edition runs the same scoring algorithm in TypeScript directly in the browser. Uploads remain on your device; the Python API remains available for local and container deployments. Run `npm run build` at the repository root to build the hosted edition. Browser report fingerprints use compact sorted JSON and are stable within this runtime; their encoding differs from Python fingerprints. Run `npm run test:parity --prefix frontend` to compare browser rankings and enrichment calculations against the Python reference.
